...
import sh4dow18.miteve_api.services.role.RoleService
...
class RoleController(private val roleService: RoleService) {
    ...
    @ResponseBody
    fun insert(@RequestBody request: RoleRequest) = roleService.insert(request)
    @PutMapping("{id}", consumes = [MediaType.APPLICATION_JSON_VALUE], produces = [MediaType.APPLICATION_JSON_VALUE])
    @ResponseBody
    fun update(@PathVariable id: Long, @RequestBody request: UpdateRoleRequest) = roleService.update(id, request)
    @DeleteMapping("{id}")
